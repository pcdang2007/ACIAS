...
$$X'_{i} = \frac{X_{i} - \min(X)}{\max(X)-\min(X)}, \quad
Y'_{i} = \frac{Y_{i} - \min(Y)}{\max(Y)-\min(Y)}, \quad
Z'_{i} = \frac{Z_{i} - \min(Z)}{\max(Z)-\min(Z)}$$ <br>
...
$$S_{i} = \alpha \times Z'_{i} + \beta \times X'_{i} + \gamma \times \frac{A_{i}}{\max(A)} \times Y'_{i} + \delta \times \frac{1 - A_{i}}{\max(A)} \times (1 - Y'_{i})$$ <br>
trong đó : 
