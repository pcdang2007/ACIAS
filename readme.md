...
$$X'_{i} = \frac{\max(X)-X_{i}}{\max(X)-\min(X)}; \quad Y'_{i} = \frac{Y_{i}-\min(Y)}{\max(Y)-\min(Y)}; \quad Z'_{i} = \frac{\max(Z)-Z_{i}}{\max(Z)-\min(Z)}$$ <br>
...
$$S_{i} = \alpha \times Z'_{i} + \beta \times X'_{i} + \gamma \times \frac{A}{A_{\max}} \times Y'_{i} + \delta \times \frac{1 - A}{A_{\max}} \times (1 - Y'_{i})$$ <br>
trong đó : 
